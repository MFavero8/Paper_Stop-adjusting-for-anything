...
The data-generating process is described via the directed acyclic graph (DAG) in @fig-DAG.ex5-1. In this DAG, acoustic sensation $(\text{C})$ is influenced directly by noise $(\text{N})$, biological sex $(\text{S})$ and age $(\text{A})$. Paticipation $(\text{P})$ in the survey response is affected by biological sex $(\text{S})$ and age $(\text{A})$.
...
ggplot(data = DAG.ex5, aes(x = x, y = y, xend = xend, yend = yend)) +
  ...
  geom_dag_text(colour = 'black', size = 10, parse = TRUE,
                family = c('mono'),
                label = c(expression(bold(A)), expression(bold(C)), expression(bold(N)), expression(bold(P)), expression(bold(S)))) +
...
-   $P \sim f_{P}(A, A)$, read as 'participation $(\text{P})$ is some function of biological sex $(\text{S})$ and age $(\text{A})$'.
...
The first sample (i.e., `ex5_sample.random`) will use simple random sampling, whereas the second one (i.e., `ex5_sample.bias`) will use non-random sampling. In this case, we will first select only the observations with based on `P == 1` and subsequently sampling randomly from them. As a result, the sample obtained is biased.
...
data.frame(colA = c('Research question', rep('Assumptions', 5), rep('Variables', 4)),
           colB = c('Total average causal effect (ACE) of biological sex (S) on acoustic sensation (C).',
                    ...
                    "Acoustic sensation (C): continous variable [unit: -]")) %>%
  group_by(colA) %>% 
...
We can also visualize all the 1,000 the estimates of the coefficient for biological sex for `Model.1`, `Model.2` and `Model.3` (i.e., the white dots in @fig-DAG.ex5-CI).
...
The bias occurs because there is selection bias. Generally, selection bias takes place when the participants selected for a study, are not representative of the larger population intended to be analysed. In our example, participation is not random but influenced by variables that also affect the outcome, specifically, biological sex $(\text{S})$ and age $(\text{A})$. As a result, biological sex $(\text{S})$ and age $(\text{A})$ are associated in the biased sample, while they are not in the population (and consequently the random sample).
...
The results from the bootstrapping are the summarized using the `summary_bootstrap()` function.
...
For the random sample, the age difference between males and female is `r sprintf(summary_sample.random[3, 'estimate'], fmt = '%#.2f')` with 95% CI \[`r sprintf(summary_sample.random[3, 'CI.lower'], fmt = '%#.2f')`, `r sprintf(summary_sample.random[3, 'CI.upper'], fmt = '%#.2f')`\]. Since the 95% CI includes zero, the average age females is not statistically significantly different from the average age of males, at the 0.05 level. Instead, for the biased sample, the age difference between males and female is `r sprintf(summary_sample.bias[3, 'estimate'], fmt = '%#.2f')` with 95% CI \[`r sprintf(summary_sample.bias[3, 'CI.lower'], fmt = '%#.2f')`, `r sprintf(summary_sample.bias[3, 'CI.upper'], fmt = '%#.2f')`\]. Since the 95% CI does not include zero, the average age females is statistically significantly different from the average age of males, at the 0.05 level.

However, once we adjust for age $(\text{A})$ in the regression model (i.e., using `Model.2`) the bias disappear. We can visualize this in @fig-DAG.ex5-regression

```{r}
#| label: fig-DAG.ex5-regression
#| fig-cap: 'Visualization of the relationship biological sex and acoustic sensation conditionning on age.'
#| fig-cap-location: bottom
#| fig-align: center
#| fig-dpi: 900
#| fig-width: 6
...
@fig-DAG.ex5-estimate-Bayesian shows the estimates (mean and 95 HDI) of the coefficient for tint for `Model.1`, `Model.2`, and `Model.3`.
...
@fig-DAG.ex5-association-Bayesian shows the relationship between biological sex $(\text{S})$ and age $(\text{A})$ in the biased and random sample. The horizontal dashed black line represent the average age in the population (i.e., `r sprintf(truth_line$A, fmt = '%#.2f')` years) for both males and females. In the random sample biological sex $(\text{S})$ and age $(\text{A})$ are not associated and the average age for males (i.e., `r sprintf(filter(mean_val, S == 'male' & sample == 'random')$A, fmt = '%#.2f')` years) and females (i.e., `r sprintf(filter(mean_val, S == 'female' & sample == 'random')$A, fmt = '%#.2f')` years) are similar to one another with a difference of `r sprintf(filter(mean_val, S == 'female' & sample == 'random')$A - filter(mean_val, S == 'male' & sample == 'random')$A, fmt = '%#.2f')` years. However, due to selection bias, the average age for males and females in the biased sample is clearly different from the population average. Additionally, biological sex $(\text{S})$ and age $(\text{A})$ are associated because females are more likely to be selected than males, and older people are less likely to be selected than younger people. As a result, females in the biased sample are, on average, older than males (average of `r sprintf(filter(mean_val, S == 'female' & sample == 'biased')$A, fmt = '%#.2f')` years for females, `r sprintf(filter(mean_val, S == 'male' & sample == 'biased')$A, fmt = '%#.2f')` years for males and a difference of `r sprintf(filter(mean_val, S == 'female' & sample == 'biased')$A - filter(mean_val, S == 'male' & sample == 'biased')$A, fmt = '%#.2f')` years).
...
#| fig-cap: 'Age difference between males and female in the biased and random sample.'
#| fig-cap-location: bottom
#| fig-align: center
#| fig-dpi: 900
#| fig-width: 6
...
For the random sample, the age difference between males and female has a mean of `r sprintf(plot.post.corr_mean.hdi[2, 'diff'], fmt = '%#.2f')` with 95% HDI \[`r sprintf(plot.post.corr_mean.hdi[2, 'diff.lower'], fmt = '%#.2f')`, `r sprintf(plot.post.corr_mean.hdi[2, 'diff.upper'], fmt = '%#.2f')`\]. Instead, for the biased sample, the age difference between males and female has a mean of `r sprintf(plot.post.corr_mean.hdi[1, 'diff'], fmt = '%#.2f')` with 95% HDI \[`r sprintf(plot.post.corr_mean.hdi[1, 'diff.lower'], fmt = '%#.2f')`, `r sprintf(plot.post.corr_mean.hdi[1, 'diff.upper'], fmt = '%#.2f')`\].

However, once we adjust for age $(\text{A})$ in the regression model (i.e., using `Model.2`) the bias disappear. We can visualize this in @fig-DAG.ex5-regression-Bayesian.
