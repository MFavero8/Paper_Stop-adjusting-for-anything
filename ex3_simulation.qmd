...
The data-generating process is described via the directed acyclic graph (DAG) in @fig-DAG.ex3-1. In this DAG, window shading $(\text{W})$ influences perceived warmth $(\text{P})$ both directly and indirectly, passing through daylight $(\text{D})$. Additionally, window shading $(\text{W})$ also indirectly influences visual comfort $(\text{V})$ through daylight $(\text{D})$.
...
data.frame(colA = c('Research question', rep('Assumptions', 6), rep('Variables', 4)),
           colB = c('Total average causal effect (ACE) of window shading (W) on perceived warmth (P).',
...
The resulting p-value is `r sprintf('%.3g', car::linearHypothesis(ex3_Model.2, 'W = 0.5')[2,'Pr(>F)'])`, which indicates that we can reject the null hypothesis (i.e., `W = 0.5`) at the 0.05 level. This result suggests that the regression coefficient for window shading (i.e., `r sprintf(coef(ex3_Model.2)['W'], fmt = '%#.3f')`) is statistically significantly different from 0.5. Since the estimated causal effect from `Model.2` is biased, we would erroneously conclude that an increase of unit in the opening of window shading causes a decrease in perceived warmth by `r sprintf(coef(ex3_Model.2)['W'], fmt = '%#.3f')` units (95% CI \[`r sprintf(confint(ex3_Model.2, level = 0.95, type = 'Wald')['W', ], fmt = '%#.3f')`\]).

For `Model.3` we found a negative coefficient between window shading $(\text{W})$ and perceived warmth $(\text{P})$ equal to `r sprintf(coef(ex3_Model.3)['W'], fmt = '%#.3f')` with 95% CI \[`r sprintf(confint(ex3_Model.3, level = 0.95, type = 'Wald')['W', ], fmt = '%#.3f')`\]. Since the 95% CI excludes zero, the regression coefficient is statistically significantly different from zero at the 0.05 level (p-value = `r sprintf('%.3g', summary(ex3_Model.3)$coef['W', 4])`). Additionally, since the 95% CI does not include the data-generating parameter for window shading (i.e., `b_win = 0.5`), we can deduce that the estimated coefficient for window shading is statistically significantly different from 0.5 at the 0.05 level. We can test this more formally by using the `linearHypothesis()` function. Specifically,
...
The resulting p-value is `r sprintf('%.3g', car::linearHypothesis(ex3_Model.3, 'W = 0.5')[2,'Pr(>F)'])`, which indicates that we can reject the null hypothesis (i.e., `W = 0.5`) at the 0.05 level. This result suggests that the regression coefficient for window shading (i.e., `r sprintf(coef(ex3_Model.3)['W'], fmt = '%#.3f')`) is statistically significantly different from 0.5. Since the estimated causal effect from `Model.3` is biased, we would erroneously conclude that an increase of unit in the opening of window shading causes a decrease in perceived warmth by `r sprintf(coef(ex3_Model.3)['W'], fmt = '%#.3f')` units (95% CI \[`r sprintf(confint(ex3_Model.3, level = 0.95, type = 'Wald')['W', ], fmt = '%#.3f')`\]).
...
For `Model.1` we found a positive coefficient between window shading $(\text{W})$ and perceived warmth $(\text{P})$ (mean = `r sprintf(plot.post_mean.hdi[1, 'W'], fmt = '%#.3f')`, 95% HDI \[`r sprintf(plot.post_mean.hdi[1, '.lower'], fmt = '%#.3f')`, `r sprintf(plot.post_mean.hdi[1, '.upper'], fmt = '%#.3f')`\]). The estimated causal effect is unbiased, leading to the correct conclusion that an increase of unit in the opening of window shading causes an increase in perceived warmth by `r sprintf(plot.post_mean.hdi[1, 'W'], fmt = '%#.3f')` units, on average.

For `Model.2` we found a negative coefficient between window shading $(\text{W})$ and perceived warmth $(\text{P})$ (mean = `r sprintf(plot.post_mean.hdi[2, 'W'], fmt = '%#.3f')`, 95% HDI \[`r sprintf(plot.post_mean.hdi[2, '.lower'], fmt = '%#.3f')`, `r sprintf(plot.post_mean.hdi[2, '.upper'], fmt = '%#.3f')`\]). The estimated causal effect is therefore biased, leading to the wrong conclusion that an increase of unit in the opening of window shading causes a decrease in perceived warmth by `r sprintf(plot.post_mean.hdi[2, 'W'], fmt = '%#.3f')` units, on average.

For `Model.3` we found a negative coefficient between window shading $(\text{W})$ and perceived warmth $(\text{P})$ (mean = `r sprintf(plot.post_mean.hdi[3, 'W'], fmt = '%#.3f')`, 95% HDI \[`r sprintf(plot.post_mean.hdi[3, '.lower'], fmt = '%#.3f')`, `r sprintf(plot.post_mean.hdi[3, '.upper'], fmt = '%#.3f')`\]). The estimated causal effect is therefore biased, leading to the wrong conclusion that an increase of unit in the opening of window shading causes a decrease in perceived warmth by `r sprintf(plot.post_mean.hdi[3, 'W'], fmt = '%#.3f')` units, on average.
