...
ggplot(data = DAG.ex3, aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_dag_text(colour = 'black', size = 10, parse = TRUE,
                family = c('mono'),
                label = c(expression(bold(D)), expression(bold(P)), expression(bold(V)), expression(bold(W)))) +
...
Applying the backdoor criterion revealed the absence of any backdoor path (i.e., a non-causal path) from window $(\text{W})$ to perceived warmth $(\text{P})$. As a result, there is no confounding and no adjustement is required.
...
data.frame(colA = c('Research question', rep('Assumptions', 6), rep('Variables', 4)),
           colB = c('Total average causal effect (ACE) of window opening (W) on perceived warmth (P).',
                    ...
                    "Window opening (W): continous variable [unit: -]",
                    'Daylight (D): continous variable [unit: lux]',
                    'Perceived warmth (P): continous variable [unit: -]',
                    "Visual comfort (V): continous variable [unit: -]")) %>%
  group_by(colA) %>% 
...
We can also visualize all the 1,000 the estimates of the coefficient for window shading for `Model.1`, `Model.2` and `Model.3` (i.e., the white dots in @fig-DAG.ex3-CI).
